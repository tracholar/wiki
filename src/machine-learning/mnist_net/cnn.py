...
class CNN(object):
    def __init__(self):
        ...
        self.W1 = tf.Variable(initial_value=np.random.randn(5, 5, 1, 50)/5, dtype=tf.float32, name='W1')
        self.b1 = tf.Variable(initial_value=np.random.randn(1, 50)/5, dtype=tf.float32, name='b1')

        self.W2 = tf.Variable(initial_value=np.random.randn(5, 5, 50, 32)/25/np.sqrt(2), dtype=tf.float32, name='W2')
        self.b2 = tf.Variable(initial_value=np.random.randn(1, 32)/25/np.sqrt(2), dtype=tf.float32, name='b2')

        z = tf.nn.conv2d(self.x, self.W1, strides=[1, 1, 1, 1], padding='SAME') + self.b1
        ...
        self.W3 = tf.Variable(initial_value=np.random.randn(4*4*32,10)/16/np.sqrt(2), dtype=tf.float32, name='W3')
        self.b3 = tf.Variable(initial_value=np.random.randn(1, 10)/16/np.sqrt(2), dtype=tf.float32, name='b3')
        ...
        self.summary_writer = tf.summary.FileWriter('./logs/cnn_adam2', self.sess.graph)
