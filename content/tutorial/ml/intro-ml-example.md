...
在搜索排序中，同样会有上述问题，你在京东上搜索手机时，也会根据你的点击率给你排个序。与广告不同的是，电子商务排序中还会考虑你的下单率，因为在电子商务中，下单才能带来实实在在的收入。在推荐系统中，推荐位置同样是有限的，推荐哪些商品和不推荐哪些商品都是需要用户的个性化估计点击率和下单率。最终根据点击率和下单率加权取得分最高的商品展示在推荐位置上。

点击率预估和下单率预估的方法没有太大差异，这里以广告点击率预估为例进行说明。广告运营商根据用户和广告的历史交互数据，可以知道用户是否点击了某个广告。然后可以找出用户的一些信息，例如性别、年龄等人口学属性，还有广告的标签和分类等信息。利用机器学习的算法，可以建立一个数学模型，该模型可以根据这些信息计算出用户的个性化点击率。到目前为止，点击率预估已被广泛地应用到广告、搜索、推荐等需要个性化的场景之中。
...
决策树是这样一种模型，如下图所示，它由很多节点构成，包括中间节点(椭圆形)和叶子节点(方形)。中间节点是一个规则，例如最上面的中间节点（也叫做根节点）的规则是花瓣长度小于2.8厘米。如果规则满足，则进入左边的叶子节点，否则就沿着右子树继续判断。叶子节点对应模型的输出结果，最左边的叶子节点输出的是setosa，表明满足该叶子节点规则就预测为setosa。每一个叶子节点都对应一条从根节点出发的路径，路径上长度称作叶子节点的深度，叶子节点的最大深度称作树的深度，图中这棵树的深度为2。一个样本过来，沿着根节点开始，不断地按照规则往下移动，直到到达叶子节点。叶子节点对应的输出结果就是模型对这个样本的预测结果。这些规则可以用一棵树状的图描述，因此叫做决策树模型。我们前面总结的规则也可以通过决策树表示出来，如下图所示。

![决策树](/wiki/static/images/iris-dt.png)



决策树节点上的规则可以通过算法自动地发现，或者说可以通过算法自动地生成一棵决策树。决策树如何发现这些规则我们暂时不要去深究，作为一个入门课程，我们重点是了解模型能干啥。这里，我们利用 scikit-learn 软件包里面的决策树模型工具，建立模型。
