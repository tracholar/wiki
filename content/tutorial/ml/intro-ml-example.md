...
决策树是这样一种模型，如下图所示，它由很多节点构成，包括中间节点(椭圆形)和叶子节点(方形)。中间节点是一个规则，例如最上面的中间节点的规则是花瓣长度小于2.8厘米。如果规则满足，则进入左边的叶子节点，否则就沿着右子树继续判断。叶子节点对应模型的输出结果，最左边的叶子节点输出的是setosa，表明满足该叶子节点规则就预测为setosa。一个样本过来，沿着根节点开始，不断地按照规则往下移动，直到到达叶子节点。叶子节点对应的输出结果就是模型对这个样本的预测结果。这些规则可以用一棵树状的图描述，因此叫做决策树模型。
